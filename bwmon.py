...
class BandwidthMonitor(app_manager.RyuApp):
	...
	port = None	
        if self.topo.edgeSwitches.get(name,port):
		for state in body:
			edge_ports = self.topo.ports[name].keys()
			if state.port_no in edge_ports:
				host = self.topo.ports[name][state.port_no]
				if self.topo.hosts.get(host,None):
					self.bwstats.addHostBwStat(self.topo.hosts[host].name, state.tx_bytes, state.rx_bytes)
